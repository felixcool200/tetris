...
namespace {
    ...
    constexpr int SCREEN_WIDTH  = (tetris::BOARD_WIDTH + tetris::BORDER_LEFT + tetris::BORDER_RIGHT) * TILE_SIZE;
    constexpr int SCREEN_HEIGHT = (tetris::BOARD_HEIGHT+ tetris::BORDER_TOP + tetris::BORDER_BOTTOM) * TILE_SIZE;
    
    // Mapping from tetris::Color to Raylib Color
    Color toRaylibColor(tetris::Color color) {
        using enum tetris::Color;
        switch (color) {
            case TETROMINO_RED:      return RED;
            case TETROMINO_GREEN:    return GREEN;
            case TETROMINO_YELLOW:   return YELLOW;
            case TETROMINO_BLUE:     return BLUE;
            case TETROMINO_MAGENTA:  return MAGENTA;
            case TETROMINO_CYAN:     return SKYBLUE;
            case TETROMINO_WHITE:    return RAYWHITE;
            case TETROMINO_BLACK:    return BLACK;
            
            // Text colors
            case TEXT_WHITE:         return WHITE;
            case TEXT_RED:           return MAROON;
            case TEXT_GREEN:         return LIME;
            case TEXT_YELLOW:        return GOLD;
            case TEXT_BLUE:          return DARKBLUE; // WAS BLUE
            case TEXT_MAGENTA:       return PURPLE;
            case TEXT_CYAN:          return VIOLET;
            case TEXT_BLACK:         return DARKGRAY;
...
Screen::StatusCode RaylibScreen::addCharAt(char ch, int x, int y, tetris::Color color) {
    ...
    DrawText(TextFormat("%c", ch), static_cast<int>(pos.x), static_cast<int>(pos.y), TILE_SIZE, toRaylibColor(color));
    return Screen::StatusCode::OKEY;
}
...
Screen::StatusCode RaylibScreen::addStringAt(std::string_view s, int x, int y, tetris::Color color) {
...
Screen::StatusCode RaylibScreen::addStringAtBoard(std::string_view s, int x, int y, tetris::Color color) {
    if (Game<RaylibScreen>::isOnBoard(x, y)) {
